...
public class UserServiceImpl implements UserService {
    ...
    public void insertUser(User user) {
        ...
        int userInfoResult = uStore.insertUserInfo(user);
    ...
    public int insertUserInfo(User user) {
        int result = uStore.insertUserInfo(user);
        return result;
    }
    ...
    public User checkUserId(String userId) {
        User idCheck = uStore.selectOneId(userId);
        return idCheck;
    }

    @Override
    public User checkUserNickname(String userNickname) {
        User nicknameCheck = uStore.selectOneNickname(userNickname);
        return nicknameCheck;
    }

    @Override
    public User checkUserEmail(String userEmail) {
        User emailCheck = uStore.selectOneEmail(userEmail);
        return emailCheck;
    }

    @Override
    public User loginUser(User user) {
        User login = uStore.selectUserLogin(user);
        return login;
    }

    @Override
    public User selectUserAllInfo(int userNo) {
        User user = uStore.selectUser(userNo);
        User userInfo = uStore.selectUserInfo(userNo);
    ...
    public void updateUserNormal(User userInfo) {
        int result = uStore.updateUserNormal(userInfo);
        int userInfoResult = uStore.updateUserInfoNormal(userInfo);
    ...
    public int deleteUser(int userNo) {
        // USER_TBL 수정
        int userResult = uStore.deleteUser(userNo);
        // USER_INFO_TBL 삭제
        int userInfoResult = uStore.deleteUserInfo(userNo);
    ...
    public List<UserMypageDto> selectUserActivity(final String userId) {
        return this.uStore.selectUserActivity(userId);
    }
}
